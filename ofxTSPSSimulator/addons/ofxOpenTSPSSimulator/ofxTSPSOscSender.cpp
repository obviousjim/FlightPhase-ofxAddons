...
void ofxTSPSOscSender::personEntered ( ofxTSPSPerson * p, ofPoint centroid, int cameraWidth, int cameraHeight, bool bSendContours ){
	...
	ofPoint velocity = p->getVelocityNormalized(cameraWidth, cameraHeight);
...
void ofxTSPSOscSender::personUpdated ( ofxTSPSPerson * p, ofPoint centroid, int cameraWidth, int cameraHeight, bool bSendContours ){
	...
	//cout << "sending velocity " << velocity.x << " " << velocity.y << endl;
...
void ofxTSPSOscSender::sendScene(ofxTSPSScene* scene) {
	ofxOscMessage m;
	m.setAddress("TSPS/scene/");
	float idleTime = scene->idleTime() / 1000.;
	m.addFloatArg( idleTime );
	m.addFloatArg( scene->percentCovered );
	send(m);
}
