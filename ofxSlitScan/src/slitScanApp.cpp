...
void slitScanApp::setup(){
	...
	sampleMapStrings.push_back("left_to_right.png");
	sampleMapStrings.push_back("right_to_left.png");
	sampleMapStrings.push_back("up_to_down.png");
	sampleMapStrings.push_back("down_to_up.png");
	sampleMapStrings.push_back("hard_noise.png");
	sampleMapStrings.push_back("soft_noise.png");
	sampleMapStrings.push_back("random_grid.png");
	sampleMapStrings.push_back("video_delay.png");
	...
	//load buttons
	loadYourOwn.loadImage("loadyourown.png");
	buttonLiveOn.loadImage("live_on.png");
	buttonMovieOn.loadImage("movie_on.png");
	buttonPauseOn.loadImage("pause_on.png");
	buttonPauseOff.loadImage("pause_off.png");
	buttonBlendingOn.loadImage("blending_on.png");
	buttonBlendingOff.loadImage("blending_off.png");
	buttonFullscreenOn.loadImage("fullscreen_on.png");
	buttonFullscreenOff.loadImage("fullscreen_off.png");
	
	leftHandle.loadImage("handle_left.png");
	rightHandle.loadImage("handle_right.png");

	frameCapacityBar.loadImage("capacity_full.png");
	frameCapacityText.loadImage("frame_capacity.png");
	
	bottomImage.loadImage("bottom_banner.png");
}
...
void slitScanApp::initMovie()
{
	...
	string file = dialog.getStringFromDialog(kDialogFile, "open movie", "select a movie file");
	if (file != "") {
		if(useLiveVideo){
			useLiveVideo = false;
			vidGrabber.close();
		...
		for(int i = 0; i <= sampleMaps.size(); i++){
			...
			if(i == currentSampleMapIndex){
				ofPushStyle();{
					...
					ofNoFill();
					ofSetColor(COLOR_YELLOW_HIGHLIGHT);
					ofRect(FRAME_PADDING/2 + i*(MAP_THUMB_WIDTH+FRAME_PADDING), 
						   FRAME_PADDING/2, 
						   MAP_THUMB_WIDTH+FRAME_PADDING, 
						   MAP_THUMB_HEIGHT+FRAME_PADDING);
...
{
	ofxFileDialog dialog;
	string mapfile = dialog.getStringFromDialog(kDialogFile, "open warp map", "select a warp map");
	if(mapfile == ""){
		return false;
	}
